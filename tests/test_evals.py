...
async def test_shows_menu_via_tool() -> None:
    ...
    async with AgentSession[CartData](llm=_llm(), userdata=CartData()) as session:
        ...
        await result.expect.contains_message(role="assistant").judge(
            judge,
            intent=(
                "Природно представляє доступні піци з меню (хоча б кілька), "
                "спираючись на дані інструмента, і допомагає з вибором. "
                "Не зобов'язана перелічувати всі позиції чи ціни."
            ),
        )
